...
#pragma region D3D9Device
	...
	HRESULT d3d9ex::D3D9Device::Present(CONST RECT* pSourceRect, CONST RECT* pDestRect, HWND hDestWindowOverride, CONST RGNDATA* pDirtyRegion)
	{
		...
		ffp.on_present();
		if (const auto inject = ignition_inject::get()) {
			inject->on_present();
		}
	...
	HRESULT d3d9ex::D3D9Device::UpdateSurface(IDirect3DSurface9* pSourceSurface, CONST RECT* pSourceRect, IDirect3DSurface9* pDestinationSurface, CONST POINT* pDestPoint)
	{
		...
		++ignition_inject::s_census.update_surface;
	...
	HRESULT d3d9ex::D3D9Device::StretchRect(IDirect3DSurface9* pSourceSurface, CONST RECT* pSourceRect, IDirect3DSurface9* pDestSurface, CONST RECT* pDestRect, D3DTEXTUREFILTERTYPE Filter)
	{
		...
		++ignition_inject::s_census.stretch_rect;

		// nGlide's once-per-frame composite of its offscreen surface onto the back buffer.
		// Letting it through paints the game's raster image over Remix's output.
		if (ignition_inject::suppress_game_blit())
		{
			IDirect3DSurface9* back_buffer = nullptr;
			if (SUCCEEDED(m_pIDirect3DDevice9->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &back_buffer))
				&& back_buffer)
			{
				const bool to_back_buffer = (pDestSurface == back_buffer);
				back_buffer->Release();
				if (to_back_buffer) {
					return D3D_OK;
				}
	...
	HRESULT d3d9ex::D3D9Device::ColorFill(IDirect3DSurface9* pSurface, CONST RECT* pRect, D3DCOLOR color)
	{
		...
		++ignition_inject::s_census.color_fill;
	...
	HRESULT d3d9ex::D3D9Device::SetRenderTarget(DWORD RenderTargetIndex, IDirect3DSurface9* pRenderTarget)
	{
		...
		++ignition_inject::s_census.set_render_target;
		if (RenderTargetIndex != 0) ++ignition_inject::s_census.set_rt_nonzero;
	...
	HRESULT d3d9ex::D3D9Device::BeginScene()
	{
		...
		const HRESULT hr = m_pIDirect3DDevice9->BeginScene();

		// Must be inside the scene and ahead of nGlide's draws: its fullscreen blit reads as
		// UI to Remix and triggers RTX injection, after which nothing else is traced.
		if (SUCCEEDED(hr)) {
			if (const auto inject = ignition_inject::get()) {
				inject->on_begin_scene_submit();
			}
		}
