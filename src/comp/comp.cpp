...
#include "modules/ignition_inject.hpp"
...
namespace comp
{
	void on_begin_scene_cb()
	{
		...
		// ignition_inject sets WORLD, VIEW and PROJECTION itself for every injected draw,
		// so nothing is seeded here. Its submission runs from BeginScene, after the real
		// BeginScene has opened the scene -- see ignition_inject::on_begin_scene_submit.
	}
	...
	void main()
	{
		...
		// Ignition submits nothing usable through D3D9 itself -- nGlide hands Remix
		// screen-space triangles. This module supplies the object-space geometry instead.
		shared::common::loader::module_loader::register_module(std::make_unique<ignition_inject>());
